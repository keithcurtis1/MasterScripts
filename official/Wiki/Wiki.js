...
`;
    ...
    const CSS =
    {
        container:      "width:100%; font-family:Arial, sans-serif;border:4px solid #422c26;",
        ...
        controlButton: "display:inline-block; margin:0 3px; padding:4px 8px; background:#6B3728; color:#ddd !important; border-radius:4px; text-decoration:none; font-size:12px;",
...
const getWikiBackgroundURL = () =>
{
    ...
    const buildPinKeywords = async (playerid, isGM = true) =>
    {
        ...
        for(const p of pins)
        {
            ...
            // Use isGM=false to avoid gmnotes fetches during keyword building
            const content = await getHandoutSectionHTML(handoutId, subHeader, field, false) || "";
    ...
    const filterPins = async (pins, isGM = true) =>
    {
        ...
        for(const p of pins)
        {
            ...
            const subHeader = p.get('subLink');
            const field     = p.get('subLinkType') === 'headerGM' ? 'gmnotes' : 'notes';
            const cacheKey  = `${handoutId}:${subHeader}:${field}`;

            if(!contentCache[cacheKey])
                contentCache[cacheKey] = await getHandoutSectionHTML(handoutId, subHeader, field, false) || "";

            const content = contentCache[cacheKey];
            const h5Words = extractKeywords(content, 5);
            const h6Words = extractKeywords(content, 6);
    ...
    const buildControlBar = (isGM = true) =>
    {
        ...
        if(isGM)
        {
            if(s.currentItemType === "header")
            {
                ...
                buttons += `<a href="${url}" target="_blank" style="${css.controlButton}; color:#eee!important;">Open</a>`;
            }
            else if(s.currentItemType === "pin")
            {
                ...
                if(pin && pin.get("link"))
                {
                    ...
                    buttons += `<a href="${url}" target="_blank" style="${css.controlButton}; color:#eee!important;">Edit</a>`;
            ...
            if(s.mode === "pins" && API_Meta && API_Meta.PinTool)
...
if(args.level)
{
    ...
    if(s.selectedHandout)
    {
        ...
        {
            const cleanedHTML = s.displayHTML.replace(/null(?=\s*(?:<\/[^>]+>)*\s*$)/i, "");
            sendChat("", `/w gm <div style="padding:10px;background-color:#ccc; border:1px solid #444;">${cleanedHTML}</div>`);
            return;
        }
        
        if(args['send-chat-players'] && s.displayHTML)
        {
            const cleanedHTML = s.displayHTML.replace(/null(?=\s*(?:<\/[^>]+>)*\s*$)/i, "");
            sendChat("", `<div style="padding:10px;background-color:#ccc; border:1px solid #444;">${cleanedHTML}</div>`);
        }
